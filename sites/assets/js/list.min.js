(function () {
	...
	frappe.ui.Filter = class {
		...
		set_events() {
			...
			this.filter_edit_area.find('.set-filter-and-run').on('click', function () {
				this$1.filter_edit_area.removeClass('new-filter');
				this$1.on_change();
				this$1.update_filter_tag();
			});
	...
	frappe.ui.filter_utils = {
		...
		get_relative_timespan_options: function get_relative_timespan_options() {
			return [
				{
					label: __('1 week'),
					value: '1 week',
	...
	frappe.ui.FilterGroup = class {
		...
		toggle_clear_filter() {
			var clear_filter_button = this.wrapper.find('.clear-filters');
		...
		set_events() {
			...
			this.wrapper.find('.add-filter').on('click', function () {
				this$1.add_filter(this$1.doctype, 'name')
					.then(this$1.toggle_clear_filter());
		...
		add_filters(filters) {
			...
			promises.push(function () { return this$1.toggle_clear_filter(); });
		...
		push_new_filter(args, is_new_filter) {
			var ref;

			if ( is_new_filter === void 0 ) is_new_filter=false;
		...
		filter_exists(filter_value) {
			...
			var exists = false;
			this.filters.filter(function (f) { return f.field; }).map(function (f) {
		...
		get_filters() {
			return this.filters.filter(function (f) { return f.field; }).map(function (f) {
				return f.get_value();
			});
		...
		update_filters() {
			this.filters = this.filters.filter(function (f) { return f.field; }); // remove hidden filters
	...
	frappe.templates['edit_filter'] = '<div class="filter-box">  <div class="list_filter row">   <div class="fieldname-select-area col-sm-4 ui-front form-group"></div>   <div class="col-sm-2 form-group">    <select class="condition form-control">     {% for condition in conditions %}     <option value="{{condition[0]}}">{{ condition[1] }}</option>     {% endfor %}    </select>   </div>   <div class="col-sm-4 form-group">    <div class="filter-field"></div>   </div>   <div class="col-sm-2">    <div class="filter-actions">     <a class="set-filter-and-run btn btn-sm btn-primary">      <span>{%= __("Apply") %}</span></a>     <a class="small grey remove-filter">      <span>{%= __("Remove") %}</span></a>    </div>   </div>  </div> </div> ';
	...
	frappe.ui.Tags = class {
		...
		bind() {
			...
			this.$input.keypress(function (e) {
				if(e.which == 13 || e.keyCode == 13) {
					var tagValue = frappe.utils.xss_sanitise(this$1.$input.val());
					this$1.addTag(tagValue);
					this$1.$input.val('');
				}
			});
		...
		getListElement($element, className) {
			var $li = $(("<li class=\"tags-list-item " + className + "\"></li>"));
			$element.appendTo($li);
			return $li;
		}
		...
		getTag(label) {
			...
			$removeTag.on("click", function () {
				this$1.removeTag($removeTag.attr('data-tag-label'));
				$removeTag.closest('.tags-list-item').remove();
			});

			if(this.onTagClick) {
				var $toggle_tag = $tag.find(".toggle-tag");
				$toggle_tag.on("click", function () {
					this$1.onTagClick($toggle_tag.attr('data-tag-label'));
				});
			}
	...
	frappe.ui.setup_like_popover = function ($parent, selector, check_not_liked) {
		...
		$parent.on('mouseover', selector, function() {
			...
			target_element.popover({
				...
				content: function () {
					var liked_by = target_element.attr('data-liked-by');
	...
	frappe.views.BaseList = class BaseList {
		...
		init() {
			...
			var tasks = [
				...
				this.setup_view ].map(function (fn) { return fn.bind(this$1); });
		...
		set_menu_items() {
			...
			this.menu_items.map(function (item) {
	...
	class FilterArea {
		...
		add(filters, refresh) {
			...
			var ref = this.set_standard_filter(filters);
			...
			var out = filters.reduce(function (out, filter) {
				...
				if (fields_dict[fieldname] && (condition === '=' || condition === "like")) {
					...
					out.promise = out.promise.then(
						function () { return fields_dict[fieldname].set_value(value); }
					);
				} else {
		...
		make_standard_filters() {
			...
			fields.map(function (df) { return this$1.list_view.page.add_field(df); });
		}
	...
	frappe.views.ListView = class ListView extends frappe.views.BaseList {
		...
		set_actions_menu_items() {
			...
			var actions = this.actions_menu_items.concat(this.workflow_action_menu_items);
			actions.map(function (item) {
				var $item = this$1.page.add_actions_menu_item(item.label, item.action, item.standard);
				if (item.class) {
		...
		show_restricted_list_indicator_if_applicable() {
			...
			if (match_rules_list.length) {
				this.restricted_list = $(("<button class=\"restricted-list form-group\">" + (__('Restricted')) + "</button>"))
					...
					.click(function () { return this$1.show_restrictions(match_rules_list); })
		...
		setup_columns() {
			...
			this.columns = this.columns.concat(
				fields_in_list_view
					.filter(function (df) {
						if (frappe.has_indicator(this$1.doctype) && df.fieldname === 'status') {
							return false;
						}
						if (!df.in_list_view) {
							return false;
		...
		render_tags() {
			...
			var $list_rows = this.$result.find('.list-row-container');

			this.data.forEach(function (d, i) {
				var tag_html = $("<div class='tag-row'>\n\t\t\t\t<div class='list-tag hidden-xs'></div>\n\t\t\t</div>").appendTo($list_rows.get(i));
				...
				var tag_editor = new frappe.ui.TagEditor({
					...
					list_sidebar: this$1.list_sidebar,
				...
				tag_editor.wrapper.on('click', '.tagit-label', function (e) {
					var $this = $(e.currentTarget);
					this$1.filter_area.add(this$1.doctype, '_user_tags', '=', $this.text());
				});
			});
		}
		...
		get_left_html(doc) {
			...
			return this.columns.map(function (col) { return this$1.get_column_html(col, doc); }).join('');
		}
		...
		get_subject_html(doc) {
			...
			var liked_by = JSON.parse(doc._liked_by || '[]');
			var heart_class = liked_by.includes(user) ?
		...
		setup_list_click() {
			...
			this.$result.on('click', '.list-row, .image-view-header', function (e) {
				var $target = $(e.target);
		...
		setup_like() {
			...
			this.$result.on('click', '.list-liked-by-me', function (e) {
				...
				if ($this.hasClass('active')) {
					this$1.filter_area.add(this$1.doctype, '_liked_by', 'like', '%' + frappe.session.user + '%');
				} else {
		...
		setup_tag_event() {
			...
			this.list_sidebar.parent.on('click', '.list-tag-preview', function () {
				this$1.toggle_tags();
			});
		}
		...
		set_rows_as_checked() {
			...
			$.each(this.$checks, function (i, el) {
				...
				this$1.$result.find((".list-row-checkbox[data-name='" + docname + "']")).prop('checked', true);
			});
			this.on_row_checked();
		}
		...
		get_share_url() {
			var query_params = this.get_filters_for_args().map(function (filter) {
		...
		show_list_settings() {
			...
			frappe.model.with_doctype("List View Setting", function () {
				var d = new frappe.ui.Dialog({
					title: __("Settings"),
					fields: frappe.get_meta("List View Setting").fields
				});
				d.set_values(this$1.list_view_settings);
				d.show();
				d.set_primary_action(__('Save'), function () {
					var values = d.get_values();
					frappe.call("frappe.desk.listview.set_list_settings", {doctype: this$1.doctype, values: values});
					Object.assign(this$1.list_view_settings, values);
					d.hide();
				});
			});
		}
		...
		get_workflow_action_menu_items() {
			...
			if (frappe.model.has_workflow(this.doctype)) {
				var actions = frappe.workflow.get_all_transition_actions(this.doctype);
		...
		toggle_workflow_actions() {
			...
			frappe.xcall('frappe.model.workflow.get_common_transition_actions', {
				...
				Object.keys(this$1.workflow_action_items).forEach(function (key) {
					this$1.workflow_action_items[key].toggle(actions.includes(key));
				});
			});
		}
		...
		get_actions_menu_items() {
			...
			var has_editable_fields = function (doctype) {
				return frappe.meta.get_docfields(doctype).some(function (field_doc) { return is_field_editable(field_doc); });
			};
			...
			var bulk_assignment = function () {
				return {
					label: __('Assign To'),
					action: function () { return bulk_operations.assign(this$1.get_checked_items(true), this$1.refresh); },
					standard: true
				};
			};
			...
			var bulk_assignment_rule = function () {
				return {
					...
					action: function () { return bulk_operations.apply_assignment_rule(this$1.get_checked_items(true), this$1.refresh); },
					standard: true
				};
			};
					...
					action: function () {
						...
						if (docnames.length > 0) {
							frappe.confirm(__('Cancel {0} documents?', [docnames.length]),
								function () { return bulk_operations.submit_or_cancel(docnames, 'cancel', this$1.refresh); });
						}
					},
					standard: true
				};
					...
					action: function () {
						...
						if (docnames.length > 0) {
							frappe.confirm(__('Submit {0} documents?', [docnames.length]),
								function () { return bulk_operations.submit_or_cancel(docnames, 'submit', this$1.refresh); });
						}
					},
					standard: true
				};
			...
			var bulk_edit = function () {
				return {
					...
					action: function () {
						...
						bulk_operations.edit(docnames, field_mappings, this$1.refresh);
					},
					standard: true
				};
			};
		...
		make (route) {
			...
			frappe.model.with_doctype(doctype, function () {
				if (locals['DocType'][doctype].issingle) {
					...
					var view_name = doctype !== 'File' ? route[2] : 'File';
	...
	class ListFilter {
		...
		refresh() {
			...
			this.get_list_filters()
				.then(function () {
					...
					this$1.wrapper.find('.list-link').remove();
					this$1.wrapper.append(html);
				});
		...
		bind_click_filter() {
			...
			this.wrapper.on('click', '.list-link', function (e) {
				var name = $(e.currentTarget).attr('data-name');
				this$1.list_view.filter_area.clear()
					.then(function () {
		...
		bind_remove_filter() {
			...
			this.wrapper.on('click', '.list-link .remove', function (e) {
				var $li = $(e.currentTarget).closest('.list-link');
				var name = $li.attr('data-name');
				$li.remove();
				this$1.remove_filter(name)
		...
		bind_save_filter() {
			...
			this.filter_input.$input.keydown(frappe.utils.debounce(function (e) {
	...
	frappe.views.ListSidebar = class ListSidebar {
		...
		setup_calendar_view() {
			...
			frappe.db.get_list('Calendar View', {
				...
				var calendar_views = result;
				var $link_calendar = this$1.sidebar.find('.list-link[data-view="Calendar"]');
				...
				var other_links = calendar_views.map(
					function (calendar_view) { return ("<li><a href=\"#List/" + doctype + "/Calendar/" + (calendar_view.name) + "\">\n\t\t\t\t\t" + (__(calendar_view.name)) + "</a>\n\t\t\t\t</li>"); }
				).join('');
		...
		setup_email_inbox() {
			...
			accounts.forEach(function (account) {
				...
				var display_name = ["All Accounts", "Sent Mail", "Spam", "Trash"].includes(account.email_id) ? __(account.email_id) : account.email_id;
				
				if (!divider) {
					this$1.get_divider().appendTo($dropdown);
					divider = true;
				}
	...
	frappe.views.ListGroupBy = class ListGroupBy {
		...
		make_group_by_fields_modal() {
			...
			d.set_primary_action("Save", function (ref) {
				var group_by_fields = ref.group_by_fields;

				frappe.model.user_settings.save(this$1.doctype, 'group_by_fields', group_by_fields || null);
				this$1.group_by_fields = group_by_fields ? ['assigned_to', 'owner' ].concat( group_by_fields) : ['assigned_to', 'owner'];
		...
		setup_dropdown() {
			...
			this.$wrapper.on('click', '.group-by-field', function (e){
				var dropdown = $(e.currentTarget).find('.group-by-dropdown');
				var fieldname = $(e.currentTarget).find('a').attr('data-fieldname');
				var fieldtype = $(e.currentTarget).find('a').attr('data-fieldtype');
				this$1.get_group_by_count(fieldname).then(function (field_count_list) {
					if (field_count_list.length) {
						this$1.render_dropdown_items(field_count_list, fieldtype, dropdown);
		...
		get_group_by_count(field) {
			...
			current_filters =
				current_filters.filter(function (f_arr) { return !f_arr.includes(field === 'assigned_to' ? '_assign': field); });
			...
			return frappe.call('frappe.desk.listview.get_group_by_count', args).then(function (r) {
				var field_counts = r.message || [];
				field_counts = field_counts.filter(function (f) { return f.count !== 0; });
				var current_user = field_counts.find(function (f) { return f.name === frappe.session.user; });
				field_counts = field_counts.filter(function (f) { return !['Guest', 'Administrator', frappe.session.user].includes(f.name); });
		...
		render_dropdown_items(fields, fieldtype, dropdown) {
			var get_dropdown_html = function (field) {
				var label = field.name == null ? __('Not Specified') : field.name;
			...
			var dropdown_html = standard_html + fields.map(get_dropdown_html).join('');
		...
		setup_filter_by() {
			...
			this.$wrapper.on('click', '.group-by-item', function (e) {
				var $target = $(e.currentTarget);
				var fieldname = $target.parents('.group-by-field').find('a').data('fieldname');
				var value = typeof $target.data('value') === 'string'
					? decodeURIComponent($target.data('value').trim())
					: $target.data('value');
				fieldname = fieldname === 'assigned_to' ? '_assign': fieldname;

				return this$1.list_view.filter_area.remove(fieldname)
					.then(function () {
						var operator = '=';
						if (value === '') {
							operator = 'is';
						...
						return this$1.list_view.filter_area.add(this$1.doctype, fieldname, operator, value);
					});
			});
		}
	...
	frappe.views.ImageView = class ImageView extends frappe.views.ListView {
		...
		item_details_html(item) {
			var info_fields = this.get_fields_in_list_view().map(function (el){ return el.fieldname; })  || [];
		...
		get_attached_images() {
			...
			return frappe.call({
				...
				this$1.images_map = Object.assign(this$1.images_map || {}, r.message);
			});
		}
	...
	frappe.views.GalleryView = Class.extend({
		...
		browse_images: function () {
			...
			$more_items.on('click', '.pswp__more-item', function (e) {
				var img_el = e.target;
				var index = this$1.pswp.items.findIndex(function (i) { return i.name === this$1.pswp.currItem.name; });
	...
	frappe.views.KanbanView.setup_dropdown_in_sidebar = function(doctype, $dropdown) {
		...
		$dropdown.on('click', '.new-kanban-board', function () {
		...
		function new_kanban_dialog() {
			...
			var fields = get_fields_for_dialog();

			var primary_action_label = fields.length > 1 ? __('Save') : '';
			var primary_action = fields.length > 1 ?
				function (ref) {
					var board_name = ref.board_name;
					var field_name = ref.field_name;
	...
	frappe.views.InboxView = class InboxView extends frappe.views.ListView {
		...
		get_meta_html(email) {
			...
			var form_link = frappe.utils.get_form_link(email.reference_doctype, email.reference_name);
			var link = email.reference_doctype && email.reference_doctype !== this.doctype ?
	...
	frappe.views.FileView = class FileView extends frappe.views.ListView {
		...
		file_menu_items() {
			...
			var items = [
				...
				},
				{
					label: __('Cut'),
					action: function () {
						frappe.file_manager.cut(this$1.get_checked_items(), this$1.current_folder);
					},
				...
				},
				{
					label: __('Paste'),
					action: function () {
						frappe.file_manager.paste(this$1.current_folder);
					},
				...
				},
				{
					label: __('New Folder'),
					action: function () {
						frappe.prompt(__('Name'), function (values) {
		...
		set_fields() {
			this.fields = this.meta.fields
				.filter(function (df) { return frappe.model.is_value_type(df.fieldtype) && !df.hidden; })
		...
		get_left_html(file) {
			...
			var file_size = file.file_size ? frappe.form.formatters.FileSize(file.file_size) : '';
		...
		toggle_cut_paste_buttons() {
			// paste btn
			var $paste_btn = this.page.menu_btn_group.find('.paste-menu-button');
			var hide = !frappe.file_manager.can_paste ||
			...
			// cut btn
			var $cut_btn = this.page.menu_btn_group.find('.cut-menu-button');
	...
	frappe.templates['kanban_card'] = '<div class="kanban-card-wrapper {{ disable_click }}" data-name="{{name}}">  <a class="kanban-card-redirect" href="#">   <div class="kanban-card content">    <div class="kanban-card-title">     {{ title }}    </div>    <div class="kanban-card-meta">    </div>   </div>  </a> </div>';
