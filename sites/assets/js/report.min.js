var Rollup = (function (exports) {
	...
	frappe.views.ReportView = class ReportView extends frappe.views.ListView {
		...
		add_currency_dependant_column(fieldname, doctype, col_index) {
			...
			if (df && df.fieldtype === 'Currency' && df.options) {
				var dependant_fieldname = df.options.includes(':')
					? df.options.split(":")[1]
					: df.options;
				if (frappe.meta.has_field(doctype, dependant_fieldname)) {
					var field = [dependant_fieldname, doctype];
					...
					action: function () {
						var d = new frappe.ui.Dialog({
							...
							primary_action: function (values) {
								...
								this$1.fields.map(function (f) { return this$1.add_currency_dependant_column(f[0], f[1]); });
			...
			if (frappe.model.can_export(this.doctype)) {
				items.push({
					...
					action: function () {
						...
						if (this$1.total_count > args.page_length) {
							fields.push({
								fieldtype: 'Check',
	...
	frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
				...
				if (data.result && data.result.length) {
					...
					if(chart_options) {
						...
						if (this$1.chart_fields) {
							var chart_options$1 = this$1.make_chart_options(this$1.chart_fields);
							chart_options$1 && this$1.render_chart(chart_options$1);
						}
					}
		...
		get_queued_prepared_reports_warning_message(reports) {
			var route = "#List/Prepared Report/List?status=Queued&report_name=" + (this.report_name);
			...
			var get_item_html = function (item) { return ("<a class=\"underline\" href=\"#Form/Prepared Report/" + (item.name) + "\">" + (item.name) + "</a>"); };
		...
		make_chart_options(ref) {
			...
			var columns = this.columns;
			var rows =  this.raw_data.result.filter(function (value) { return Object.keys(value).length; });
		...
		get_possible_chart_options() {
			...
			function preview_chart() {
				...
				if (values.x_field && values.y_field) {
					var options = me.make_chart_options(values);
					wrapper.empty();
			...
			var dialog = new frappe.ui.Dialog({
				...
				primary_action: function (values) {
					...
					var x_field_label = numeric_fields.filter(function (field) { return field.fieldname == values.y_field; })[0].label;
					var y_field_label = non_numeric_fields.filter(function (field) { return field.fieldname == values.x_field; })[0].label;

					options.title = __(((this$1.report_name) + ": " + x_field_label + " vs " + y_field_label));
			...
			return columns.map(function (column) {
				...
				var format_cell = function (value, row, column, data) {
					if (column.isHeader && !data && this$1.data) {
						...
						var index = 1;
						if (this$1.datatable && this$1.datatable.options.checkboxColumn) { index = 2; }
							...
							primary_action: function (values) {
								...
								frappe.call({
									...
									callback: function (r) {
										...
										this$1.add_custom_column(custom_columns, custom_data, link_field, values.field, insert_after_index);
										d.hide();
									}
								});
	...
	frappe.templates['print_grid'] = ' {% if title %} <h2>{{ __(title) }}</h2> <hr> {% endif %} {% if subtitle %} {{ subtitle }} <hr> {% endif %} <table class="table table-bordered">    <thead>   <tr>   {% for col in columns %}    {% if col.name && col._id !== "_check" %}    <th     {% if col.minWidth %}      style="min-width: {{ col.minWidth }}px"     {% endif %}     {% if col.docfield && frappe.model.is_numeric_field(col.docfield) %}      class="text-right"     {% endif %}    >     {{ __(col.name) }}</th>    {% endif %}   {% endfor %}   </tr>  </thead>    <tbody>   {% for row in data %}    <tr style="height: 30px">    {% for col in columns %}     {% if col.name && col._id !== "_check" %}       {% var value = col.fieldname ? row[col.fieldname] : row[col.id]; %}       <td {% if row.bold == 1 %} style="font-weight: bold" {% endif %}>       <span {% if col._index == 0 %} style="padding-left: {%= cint(row.indent) * 2 %}em" {% endif %}>        {% format_data = row.is_total_row && ["Currency", "Float"].includes(col.fieldtype) ? data[0] : row %}        {% if (row.is_total_row && col._index == 0) { %}         {{ __("Total") }}        {% } else { %}         {{          col.formatter           ? col.formatter(row._index, col._index, value, col, format_data, true)           : col.format            ? col.format(value, row, col, format_data)            : col.docfield             ? frappe.format(value, col.docfield)             : value         }}        {% } %}       </span>      </td>     {% endif %}    {% endfor %}    </tr>   {% endfor %}  </tbody> </table> ';
	...
	frappe.templates['group_by'] = '<div class="groupby-box" style="display: none">  <div class="list_groupby row">   <div class="col-sm-4 form-group ui-front">    <select class="groupby form-control">     <option value="" disabled selected>{{ __("Select Group By...") }}</option>     {% for (var parent_doctype in groupby_conditions) { %}      {% for (var val in groupby_conditions[parent_doctype]) { %}       {% if (parent_doctype !== doctype) { %}       <option         data-doctype="{{parent_doctype}}"        value="{{groupby_conditions[parent_doctype][val].fieldname}}">        {{ groupby_conditions[parent_doctype][val].label }}        ({{ parent_doctype }})       </option>       {% } else { %}       <option         data-doctype="{{parent_doctype}}"        value="{{groupby_conditions[parent_doctype][val].fieldname}}">        {{ groupby_conditions[parent_doctype][val].label }}       </option>       {% } %}      {% } %}     {% } %}    </select>   </div>   <div class="col-sm-2 form-group">    <select class="aggregate-function form-control">     {% for condition in aggregate_function_conditions %}     <option value="{{condition.name}}">{{ condition.label }}</option>     {% endfor %}    </select>   </div>   <div class="col-sm-6 col-xs-12">    <div class="groupby-field pull-left">     <select class="aggregate-on form-control" style="display: none">      <option value="" disabled selected>{{ __("Select Field...") }}</option>     </select>    </div>    <div class="groupby-actions pull-left">     <a class="small grey remove-groupby pull-left">      <i class="octicon octicon-trashcan visible-xs"></i>      <span class="hidden-xs">{{ __("Remove") }}</span>     </a>    </div>    <div class="clearfix"></div>   </div>  </div> </div> ';
	...
	frappe.ui.GroupBy = class {
		...
		setup_group_by_area() {
			...
			this.aggregate_function_select.on('change', function () {
				this$1.show_hide_aggregate_on();
			});

			// try running on change
			this.groupby_select.on('change', function () { return this$1.apply_group_by_and_refresh(); });
			this.aggregate_function_select.on('change', function () { return this$1.apply_group_by_and_refresh(); });
			this.aggregate_on_select.on('change', function () { return this$1.apply_group_by_and_refresh(); });

			$('.set-groupby-and-run').on('click', function () {
				this$1.apply_group_by_and_refresh();
			});

			$('.remove-groupby').on('click', function () {
					...
					var loop = function ( doctype ) {
						...
						doctype_fields.forEach(function (field) {
							...
							if (frappe.model.is_numeric_field(field.fieldtype)) {
								var option_text = doctype == this$1.doctype
									? field.label
									: ((field.label) + " (" + doctype + ")");
		...
		apply_settings(settings) {
			...
			var group_by_fieldname = settings.group_by.split('.')[1].replace(/`/g, '');
		...
		set_args(args) {
			if (this.aggregate_function && this.group_by) {
				var aggregate_column, aggregate_on_field;
				if (this.aggregate_function === 'count') {
				...
				if(!this.report_view.fields.map(function (f) { return f[0]; }).includes('_aggregate_column')) {
